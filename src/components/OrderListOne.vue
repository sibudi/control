  ...
  <div style="margin:15px 15px;">
    <template>
     ...
     <el-checkbox-group v-model="otherAmount">
      <el-checkbox :label="1" @change.native="autoDistribute">其他</el-checkbox>
    </el-checkbox-group>
  </template>
  ...
  distribute() {
    ...
    this.$http.post('manage/collection/order-assignment',result).then(response =>{
     ...
     autoDistribute(row) {              //自动分配
      ...
      this.$http.post('manage/collection/overdue-assignment-statistics',{postId:this.postId,section:this.itemValue,'amountApply':this.amountApply,'otherAmount':this.appflag}).then(response => {
