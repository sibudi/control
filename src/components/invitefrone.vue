<template>
  ...
  <template>
    <el-table :data="gridData" highlight-current-row class="grid" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="状态" prop="type" min-width="120">
        <template scope="scope">
          {{getChannelNameOne(scope.row.type)}}
        </template>
      </el-table-column>
      <el-table-column label="提现时间" prop="createTime" min-width="200" sortable>
      </el-table-column>
    </el-table>
  </template>
  <!--分页-->
...
export default {
  ...
  methods: {
   ...
   bindGrid() {
    ...
    this.$http.post('manage/activity/withdrawRecord', _data).then(response => {
