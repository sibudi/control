...
<template>
  <section>
    ...
    <el-form :inline="true" :model="searchForm" class="toolbar">
      ...
      <el-form-item label="录音日期">
        <div class="block">
          <el-date-picker v-model="searchForm.recordBeginTime" type="date" :picker-options="pickerOptions1" :editable="false" placeholder="选择日期">
          </el-date-picker> ~
          <el-date-picker v-model="searchForm.recordEndTime" type="date" :picker-options="pickerOptions2" :editable="false" placeholder="选择日期">
          </el-date-picker>
        </div>
      </el-form-item><br>
      <el-form-item label="催收人员" prop="userName">
        <el-select v-model="searchForm.userName" placeholder="请选择" filterable clearable>
          <el-option v-for="item in thirdCompanyList" :label="item.name" :key="item.code" :value="item.code"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-button @click="search" type="info">查询</el-button>
        <el-button @click="downLoad" type="info">批量下载</el-button>
      </el-form-item>
    </el-form>
    ...
    <template>
      <el-table :data="gridData" ref="multipleTable" tooltip-effect="dark" @selection-change="handleSelectionChange" highlight-current-row v-loading="gridLoading"  class="grid">
        ...
        <el-table-column label="申请金额" prop="applyAmount" width="150">
        </el-table-column>
        <el-table-column label="催收人员" prop="userName" width="150">
        </el-table-column>
        <el-table-column label="录音开始时间" prop="recordBeginTime" width="150" sortable>
          <template scope="scope">
            {{getdate(scope.row.recordBeginTime)}}
          </template>
        </el-table-column>
        <el-table-column label="录音结束时间" prop="recordEndTime" width="150" sortable>
          <template scope="scope">
            {{getdate(scope.row.recordEndTime)}}
          </template>
        </el-table-column>
        <el-table-column label="录音时长" prop="recordLength" width="200" sortable>
        </el-table-column>
      ...
      <el-table-column label="下载" prop="checkResult" width="150" sortable>
        <template scope="scope">
          <el-button size="small" @click="downOnece(scope.row)">下载</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!--分页-->
