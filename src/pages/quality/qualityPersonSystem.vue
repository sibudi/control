...
<template>
	<section>
		<el-row>
			<el-table :data="gridData" highlight-current-row v-loading="gridLoading" class="grid" border>
				<el-table-column label="阶段" prop="postName"></el-table-column>
    ...
    <el-table-column label="操作">
    	<template scope="scope">
    		<el-button size="small" type="primary" @click="edit(scope.row, 1)">编辑</el-button>
...
<el-row>
	<el-form :model="qualityForm" ref="qualityForm" label-width="100px">
		<el-form-item label="催收人员" prop="outsourceId">
			<el-select v-model="qualityForm.outsourceId" placeholder="请选择" filterable clearable>
		...
		<el-form-item label="质检人员" prop="checkerId">
			<el-select v-model="qualityForm.checkerId" placeholder="请选择" filterable clearable>
		...
		<el-form-item label="质检时间">
			<div class="block">
				<el-date-picker v-model="qualityForm.startTime" type="date" :picker-options="pickerOptions1" :editable="false" placeholder="选择日期">
				</el-date-picker> ~
				<el-date-picker v-model="qualityForm.endTime" type="date" :picker-options="pickerOptions2" :editable="false" placeholder="选择日期">
				</el-date-picker>
			</div>
		</el-form-item>
		<el-form-item>
			<el-button @click="search" type="info" :loading="reprotLoading">下载报告</el-button>
		</el-form-item>
	</el-form>
</el-row>
		...
		tijiaoqua() {
			...
			this.$http.post('manage/qualityCheck/getCheckReport',_data).then(response => {
				...
				if(body.code == 1 && body.data){
					window.open('http://cn.control.do-it.id/manage/getExcelFile?path=/' + body.data);
					this.bindGrid();
