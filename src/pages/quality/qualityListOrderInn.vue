  ...
  <section>
    ...
    <el-form :inline="true" :model="searchForm" class="toolbar">
      ...
      <el-form-item label="Waktu terakhir pemeriksaan">
        <div class="block">
          <el-date-picker v-model="searchForm.updateBeginTime" type="date" :picker-options="pickerOptions1" :editable="false" placeholder="Silahkan pilih">
          </el-date-picker> ~
          <el-date-picker v-model="searchForm.updateEndTime" type="date" :picker-options="pickerOptions2" :editable="false" placeholder="Silahkan pilih">
          </el-date-picker>
        </div>
      </el-form-item><br>

      <el-form-item label="Waktu Follow Up">
        <div class="block">
          <el-date-picker v-model="searchForm.dueDayStartTime" type="date" :picker-options="pickerOptions3" :editable="false" placeholder="Silahkan pilih">
          </el-date-picker> ~
          <el-date-picker v-model="searchForm.dueDayEndTime" type="date" :picker-options="pickerOptions4" :editable="false" placeholder="Silahkan pilih">
          </el-date-picker>
        </div>
      </el-form-item>
      <el-form-item label="Petugas penagihan" prop="collectionInQualityCheckId">
        <el-select v-model="searchForm.collectionInQualityCheckId" placeholder="Silahkan pilih" filterable clearable>
          <el-option v-for="item in thirdCompanyList" :label="item.name" :key="item.code" :value="item.code"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
        <el-button @click="search" type="info">Cek</el-button>
      </el-form-item>
    </el-form>
    ...
    <template>
      <el-table :data="gridData" highlight-current-row v-loading="gridLoading" @row-click="rowClick" class="grid">
       ...
       <el-table-column label="Waktu terakhir pemeriksaan" prop="updateTime" width="150" sortable>
        <template scope="scope">
          {{getdate(scope.row.updateTime)}}
        </template>
      </el-table-column>
      <el-table-column label="Hasil pemeriksaan kualitas" prop="checkResultInn" width="200" sortable>
      </el-table-column>
      ...
      <el-table-column label="Waktu Follow Up" prop="followTime" width="150" sortable>
        <template scope="scope">
      ...
      <el-table-column label="Petugas penagihan" prop="realName" width="150">
      </el-table-column>
      <el-table-column label="Petugas penagihan" prop="collectiorName" min-width="116">
     <!--  <template scope="scope">
        <span>{{getThirdCompany(scope.row.outsourceId)}}</span>
      </template> -->
    </el-table-column>
    <el-table-column label="Jumlah pinjaman" prop="amountApply" width="150">
    </el-table-column>
    <el-table-column label="Durasi pinjaman" prop="borrowingTerm" width="150">
    </el-table-column>
    <el-table-column label="Jumlah hari keterlambatan" prop="overdueDays" width="80" sortable>
      <template scope="scope">
...
export default {
  ...
  getdate(dates) {
   ...
   var myDates = new Date(dates+offset+7*3600000);
    ...
    check(row){
      window.open('#/qualityDetailsInn?&uuid='+row.uuid);
    },
    rowClick(row){
      window.open('#/qualityDetailsInn?seen=true&userUuid='+row.uuid+
        '&uuid='+row.orderNo);
    },
    pageSizeChange(val) {
      this.pageSize = val;
