...
<template>
	<section>
        ...
        <el-table-column label="失败时间" prop="failedTime" min-width="150" sortable>
         <template scope="scope">
          <span>{{getdate(scope.row.failedTime)}}</span>
        </template>
      </el-table-column>
      <el-table-column label="失败原因" prop="errorMessage" width="250">
      </el-table-column>
      <el-table-column label="银行卡" prop="shortBankNumber" width="250">
      </el-table-column>
...
export default {
   ...
   getdate(dates) {
    ...
    // myDate.setUTCHours(10);
      ...
      check(row){
        window.open('#/OrderListDetails?seen=true&userUuid='+row.userUuid+
          '&uuid='+row.uuid);
      },
      getOrderType(type){
