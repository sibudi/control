...
const DataUtil = {
  ...
  formatUnixTimeInn(time){
    if(time == null){return '';}
    let timeStamp = new Date(time);
    return timeStamp.getFullYear() + "Tahun"
      + (timeStamp.getMonth() + 1) + "Bulan" + timeStamp.getDate() + "Tanggal"
      + timeStamp.getHours() + "Jam" + timeStamp.getMinutes() + "Menit";
  },
    /**
   * 将unix时间戳转化为yy-mm-dd*/
   ...
   * 用户的分机号码*/
  voicePhone(param){
    return this.userUtil('voicePhone',param);
  },
  ...
  employeeNumber(param){
    return this.userUtil('employeeNumber',param);
  },
  ...
  getPermissionSet() {
    ...
    // Arr.add('/CollectionOrderDetailsByIdInn');
    return Arr;
  }
};
